...
\hypersetup{colorlinks=true,linkcolor=Navy,citecolor=Teal,urlcolor=Navy,
  pdftitle={Gaussian Tensor Sketches: Two rMPS Proofs and a Mixed Reconstruction Chain},
...
\fancyhead[R]{\sffamily\small rMPS / mixed reconstruction}
...
\newcommand{\cF}{c_{\F,p}}
% P02 macros; the original rMPS notation is unchanged.
...
% centre-to-centre spacing. A comparison follows on page 2; the proofs start on page 3.
...
% A dedicated second-page comparison. No numbered counters are changed.
\section*{Three-chain comparison}
\phantomsection\label{sec:three-chain-overview}
{\sffamily\color{Teal}\large Two routes to one moment theorem; a hybrid reconstruction application.\par}
\vspace{2mm}
Chains A and B have the \emph{same} rMPS endpoint and constants. Chain C combines
Wick contractions with seeded Hermite multiplication for a \emph{different}
quantity: an inverse Gram coupled to the residual from the same sketch.
...
\toprule
\textbf{Comparison} &
{\color{Teal}\textbf{Chain A} (\S\ref{sec:A})\newline Wick / transfer} &
{\color{Navy}\textbf{Chain B} (\S\ref{sec:B})\newline Multiplication / energy} &
{\color{Amber}\textbf{Chain C} (\S\S\ref{p02:sec:model}--\ref{p02:sec:scope})\newline Mixed Wick--Hermite}\\
\midrule
\rowcolor{Ice}
\textbf{Quantity} &
Centered Gram moment $\E\tr G^4$ for independent rMPS probes. &
The same $\E\tr G^4$ for the same probe law. &
Reconstruction penalty $F=c^{\mathsf T}\Gamma^{-2}c$;
$\Delta=\Gamma-I_r$.\\[3pt]
\textbf{Geometry and field} &
Real or complex cores; every fixed two-dimensional $U$.
Exact formula on the product-prefix family. &
Exactly the same model and subspace scope as A. &
Real shared-factor Khatri--Rao sketch; specified orthogonal signal--residual
family. Rank-two certificates and fixed-rank expansion.\\[3pt]
\rowcolor{Ice}
\textbf{Retained information} &
Core pairings or permutations, bond-loop counts, and coefficient-network
contractions. &
Reachable Hermite sectors and the coefficient Gram across each
conditional core contraction. &
The seed $c$, its live sample label, and signed mixed moments
$M_j=\E[c^{\mathsf T}\Delta^jc]$ through $j=4$.\\[3pt]
\textbf{Wick's role} &
Exact joint core integration; transfer row sum
$T_p\one=\rho_p(\chi)\one$. &
Not needed to enumerate the core states; Gaussian rotation and convexity
give the same factor $\rho_p(\chi)$. &
Evaluates marked sample partitions and coefficient contractions without
separating $\Delta$ from $c$.\\[3pt]
\rowcolor{Ice}
\textbf{Framework role} &
A standalone transfer proof; no graded cutoff is needed. &
Original-law vacuum energy, exact reachable-state cutoff, one-core
contraction, and energy assembly. &
Seeded finite Hermite compression; even moments as energies and odd
moments as cross inner products.\\[3pt]
\textbf{Completion and inverse control} &
Spherical averaging and ordered fourth-order sample assembly.
No inverse is present. &
The same shared completion as A; no inverse-tail input is needed. &
Signed finite polynomial majorant on a good event. A separate inverse
moment bound justifies the unrestarted expectation.\\[3pt]
\rowcolor{Ice}
\textbf{Constants and gain} &
Scalar eighth-moment factors $105\mu_4$ (real), $24\mu_4$ (complex);
identical final constants to B. &
Same scalar, projected, and centered-Gram constants as A;
not a constants improvement over A. &
At rank two, the true correction is $-30/m^2$, not the $+198/m^2$
produced by independent replacement.\\[3pt]
\textbf{Scope limit} &
Fixed fourth centered matrix moment; not a sharp growing-rank or
logarithmic-confidence theorem. &
Same limit as A. This example does not establish stronger bounds than
Wick/transfer. &
One specified geometry; not arbitrary-geometry universality or
algorithmic optimality. Polynomial moments alone do not control inverses.\\[3pt]
\bottomrule
\end{tabularx}
\endgroup

\vspace{1.5mm}
\begin{takeaway}[title={Quantitative endpoints (real case)},
  top=3pt,bottom=3pt,before skip=3pt,after skip=3pt]
\small
\setlength{\abovedisplayskip}{3pt}
\setlength{\belowdisplayskip}{3pt}
\[
\begin{aligned}
 \text{A and B:}\quad &\E\tr G^4\le
 \frac{384\mu_4+3(N-1)(8\mu_2-2)^2}{N^3},\\[3pt]
 \text{C:}\quad &\E F=\frac6m-\frac{30}{m^2}+\frac{2598}{m^3}+O(m^{-7/2}).
\end{aligned}
\]
{\footnotesize $N$ counts probes; $m$ counts sketch rows.
The second line is the rank-two, unit-residual expansion with separate inverse
control; it is not a bound for arbitrary rMPS geometry.}
\end{takeaway}
\clearpage
...
The original rMPS comparison is followed by a separate mixed reconstruction
application: the diagram on page~\pageref{p02:fig:mixedchain} maps Chain~C,
and Sections~\ref{p02:sec:model}--\ref{p02:sec:scope} supply its proof.
This is an application of the combined Wick--Hermite organization, not a third
independent proof of the same rMPS endpoint. Appendix~\ref{p02:app:audit}
records the attribution boundaries across both applications.
...
\clearpage
\phantomsection\label{p02:fig:mixedchain}
\pdfbookmark[1]{Mixed Wick--framework reconstruction chain}{mixed-chain-map}
{\sffamily\color{Navy}\small METHOD PROBLEMS\hfill MIXED PROOF-CHAIN MAP\par}
\vspace{3mm}
{\sffamily\bfseries\fontsize{22}{26}\selectfont Retain the mixed reconstruction energy\par}
\vspace{1.5mm}
{\sffamily\large Chain C: Wick contractions inside the original-law framework\par}
\vspace{2mm}
{\small Read downward. Blue retains the state; teal evaluates Gaussian moments.\par
Amber marks the two different analytic completions, not interchangeable claims.\par}
\vspace{2mm}
\begin{center}
\begin{tikzpicture}[
 >={Latex[length=1.6mm,width=1.25mm]},
 ...
 inner xsep=6pt,inner ysep=5pt,outer sep=.5pt},
 full/.style={base,draw=Navy!70,fill=Ice,text width=\RMPSFullWidth},
 blue/.style={full,draw=Navy!85,fill=Navy!4},
 teal/.style={full,draw=Teal!85,fill=Teal!4},
 half/.style={base,text width=\RMPSColumnWidth},
 ahead/.style={half,draw=Amber!90,fill=Amber!13,minimum height=12mm,
 font=\sffamily\bfseries\fontsize{10.2}{12.4}\selectfont},
 abox/.style={half,draw=Amber!80,fill=Amber!4,minimum height=26mm},
 endpoint/.style={half,draw=Navy!75,fill=Ice,minimum height=27mm},
 flow/.style={->,draw=Navy!80,line width=.65pt},
 gflow/.style={->,draw=Teal!90,line width=.65pt},
 aflow/.style={->,draw=Amber!90,line width=.65pt}
]
\node[full,minimum height=18mm] (target)
 {\textbf{Original sketch and fixed geometry; no independent replacement}\\[2pt]
  $\Gamma=I_r+\Delta,\quad c=m^{-1}\sum_i h_i^2g_i\xi_i,
    \quad F=c^{\mathsf T}\Gamma^{-2}c$};
\node[blue,below=4mm of target,minimum height=21mm] (seed)
 {\textbf{C1. Seeded multiplication and exact Hermite compression}\\[2pt]
  $M_j=\langle c,K^jc\rangle,\quad K=\Pi\mathcal M_\Delta\Pi,
   \quad 0\le j\le4$\\[2pt]
  Two forward steps: degree $\le12$; at most three active sample blocks};
\draw[flow] (target.south)--(seed.north);
\node[teal,below=4mm of seed,minimum height=22mm] (wick)
 {\textbf{C2. Contract the residual register; Wick-evaluate marked labels}\\[2pt]
  $D=\E_\xi[cc^{\mathsf T}\mid g,h]
    =m^{-2}\sum_i h_i^4g_ig_i^{\mathsf T},\quad
    M_j=\E\tr(\Delta^jD)$\\[2pt]
  The sample label in $D$ stays live through every $\Delta$ insertion};
\draw[gflow] (seed.south)--(wick.north);
\node[full,below=4mm of wick,minimum height=22mm] (moments)
 {\textbf{C3. Keep the signed mixed moments, rather than separate norms}\\[2pt]
  $M_0=6/m,\quad M_1=114/m^2,
    \quad M_2=66/m^2+4740/m^3$\\[2pt]
  Compute $M_3,M_4$ and $\E\tr\Delta^4$ exactly as well ($r=2$)};
\draw[gflow] (wick.south)--(moments.north);
\node[ahead,below=7mm of moments.south,xshift=-\RMPSColumnOffset] (goodh)
 {C4a. GOOD-EVENT CERTIFICATE};
\node[ahead,below=7mm of moments.south,xshift=\RMPSColumnOffset] (invh)
 {C4b. ORIGINAL EXPECTATION};
\coordinate (fork) at ($(moments.south)+(0,-3mm)$);
\draw[aflow] (moments.south)--(fork)-|(goodh.north);
\draw[aflow] (moments.south)--(fork)-|(invh.north);
\node[abox,below=4mm of goodh] (good)
 {\textbf{Global positive polynomial majorant}\\[2pt]
  $q_\theta\ge0$ on $\mathbb R$;\\
  $(1+x)^{-2}\le q_\theta(x)$ for $|x|\le\theta$\\[2pt]
  Drop the indicator only from the nonnegative polynomial};
\node[abox,below=4mm of invh] (inverse)
 {\textbf{Separate inverse-moment input}\\[2pt]
  $(\E\|\Gamma^{-1}\|^8)^{1/8}\le L_r$\\
  for $m\ge32(r+2)$\\[2pt]
  Finite rational identity + H\"older; no infinite series exchange};
\draw[aflow] (goodh.south)--(good.north);
\draw[aflow] (invh.south)--(inverse.north);
\node[endpoint,below=4mm of good] (goode)
 {\textbf{Original-law good-event bound}\\[2pt]
  $\E[F\mathbf1_{\mathcal A_{1/2}}]\le u_m$\\[2pt]
  Restart: $\displaystyle\E F_{\rm acc}\le\frac{u_m}{1-d_m}$\\[2pt]
  $\E[\text{attempts}]\le(1-d_m)^{-1}$};
\node[endpoint,below=4mm of inverse] (inve)
 {\textbf{Unrestarted, fixed-rank expansion}\\[2pt]
  $\displaystyle\E F=\frac6m-\frac{30}{m^2}
      +\frac{2598}{m^3}$\\[2pt]
  ${}+O(m^{-7/2})$ for $r=2$;\\
  inverse integrability has been charged};
\draw[aflow] (good.south)--(goode.north);
\draw[aflow] (inverse.south)--(inve.north);
\node[fit=(goode)(inve),inner sep=0pt,outer sep=0pt] (bottom) {};
\node[full,below=6mm of bottom.south,draw=Amber!80,fill=Amber!6,
 minimum height=16mm] (scope)
 {\textbf{Scope: one shared-factor reconstruction geometry}\\[2pt]
  Not arbitrary-geometry universality or generalized-Nystr\"om optimality.\\
  Polynomial moments alone do not prove inverse integrability.};
\end{tikzpicture}
\end{center}
\clearpage
\section{Chain C: mixed reconstruction model and result}\label{p02:sec:model}
\begin{takeaway}[title={A new endpoint, not a third rMPS moment bound}]
The two rMPS chains above meet at a scalar moment bound. Chain~C instead
retains a \emph{mixed reconstruction energy} involving both a random Gram
inverse and a correlated residual coupling. For the rank-two, unit-residual
model below, its original-estimator conclusion is
\[
 \E F=\frac6m-\frac{30}{m^2}+\frac{2598}{m^3}+O(m^{-7/2}).
\]
The finite mixed moments and the good-event certificate do not require an
inverse-tail estimate. The unconditional conclusion does, and that input is
proved separately in Section~\ref{p02:sec:inverse}.
\end{takeaway}

\subsection{Bridge from the rMPS radial boundary; local notation}
The shared-factor row used here is a two-site, bond-one real rMPS row,
up to swapping the two physical sites. Equivalently, it is the ordinary
order-two Gaussian Khatri--Rao row already contained in the Gaussian
tensor-train family \cite{RR2020,CEMT}. Restricting the first factor to
$r+1$ coordinates gives
\[
 (y,z)=h(g,\xi),\qquad (y,z)\stackrel d=\sqrt w\,(g,\xi),\quad w=h^2.
\]
This is the common radial boundary from Section~\ref{sec:exact}, now with
one direction reserved for the residual. The two occurrences of $h$ are
shared, not separately resampled. For equal physical dimensions take
$d\ge r+1$; extra coordinates are unused. The mixed polynomial formulas with
general radial moments below can be reused for other radial laws, but the
inverse-integrability theorem in this part is proved for $w=h^2$ only.

To avoid a collision with the \emph{centered} rMPS matrix $G$ in
\eqref{eq:G}, this part denotes the reconstruction Gram by $\Gamma$ and its
centered error by $\Delta=\Gamma-I_r$. These are the quantities called
$G$ and $E$ in the P02 problem statement. Here $m$ is the sketch row count,
$r$ is the fixed subspace dimension, and $\mu_k=\E w^k=(2k-1)!!$.
There is no new assertion about arbitrary subspace--residual geometry or
general rMPS bond dimension.

\subsection{A specified orthogonal signal--residual geometry}

Fix \(r\ge1\), \(n_1\ge r+1\), \(n_2\ge2\), and a unit coordinate vector \(f_1\in\R^{n_2}\). Define
\[
 u_a=e_a\otimes f_1\quad(1\le a\le r+1),\qquad
 Q=[u_1,\ldots,u_r],\qquad v=u_{r+1}.
\]
Take, for example,
\[
 A=[Q,v],\qquad B=[I_r,0],\qquad R=v e_{r+1}^{\mathsf T},
\]
so \(A=QB+R\), \(Q^{\mathsf T}R=0\), and \(\Frob R^2=1\). More generally the signal coefficient matrix is arbitrary and the rank-one residual can be scaled. All penalty formulas below scale by \(\Frob R^2\).

Let \(a_i\sim N(0,I_{n_1})\) and \(b_i\sim N(0,I_{n_2})\) be independent over both factors and all \(i\), and put
\[
 \Psi=\frac1{\sqrt m}\begin{pmatrix}(a_1\otimes b_1)^{\mathsf T}\\ \vdots\\(a_m\otimes b_m)^{\mathsf T}\end{pmatrix}.
\]
Unused ambient coordinates play no role. The ambient dimensions may be arbitrarily large, including large enough that this is genuinely dimension-reducing.
Write
\[
 g_i=(a_{i1},\ldots,a_{ir})^{\mathsf T},\quad
 \xi_i=a_{i,r+1},\quad h_i=b_{i1},\quad w_i=h_i^2.
\]
Here \(g_i\sim N(0,I_r)\), \(\xi_i,h_i\sim N(0,1)\), and all these primitives are independent. Set
\begin{equation}\label{p02:eq:model}
 \Gamma=Q^{\mathsf T}\Psi^{\mathsf T}\Psi Q
   =\frac1m\sum_i w_i g_i g_i^{\mathsf T},\qquad
 \Delta=\Gamma-I_r,\qquad
 c=\frac1m\sum_i w_i g_i\xi_i.
\end{equation}
Then \(C=Q^{\mathsf T}\Psi^{\mathsf T}\Psi R=c e_{r+1}^{\mathsf T}\).
For \(m\ge r\), \(\Gamma\) is positive definite almost surely: all \(h_i\) are nonzero almost surely and a Gaussian \(m\times r\) matrix has full column rank.

Let \(W=\Psi Q\) and reconstruct
\(\widehat A=QW^\dagger\Psi A\). Orthogonality gives the exact identity
\begin{equation}\label{p02:eq:error}
 \Frob{A-\widehat A}^2=1+F,\qquad
 F=c^{\mathsf T}\Gamma^{-2}c.
\end{equation}
Indeed \(W^\dagger\Psi A=B+\Gamma^{-1}C\) and \(R\perp\operatorname{range}Q\).
This fixed-range reconstruction is the established approximation
$Q(\Psi Q)^\dagger\Psi A$, not a new algorithm. Tropp--Yurtsever--Udell--Cevher
\cite[equations~(4.2)--(4.3), Lemmas~A.3--A.4]{TYUC2017} give its orthogonal
error decomposition and correction term; see also
\cite[Section~5.1, Lemma~5.1]{EW}. Their Remark~4.1 records still earlier
algebraically equivalent approximations, so no algorithmic priority is assigned here.
The fully iid real Gaussian benchmark is $\E F=r/(m-r-1)$ for $m>r+1$.
It already appears for fixed-range reconstruction in
\cite[Lemma~A.5]{TYUC2017}; Bartan--Pilanci
\cite[Lemma~1 in arXiv v2]{BP2020} give the corresponding regression
prediction-error identity. Epperly--Webber \cite[Theorem~2.1]{EW}
explicitly cite Bartan--Pilanci for their real Gaussian benchmark.
The standard Gaussian and pseudoinverse inputs are also in
\cite[Propositions~10.1--10.2]{HMT}.
Our $\Psi$ is not an iid Gaussian matrix. Neither the error decomposition nor
the Gaussian benchmark is claimed as a contribution of this note.

\par\Needspace{10\baselineskip}
\section{Mixed moments with the original sample labels}\label{p02:sec:moments}
Define the original-law quantities
\[
 M_j=\E[c^{\mathsf T}\Delta^j c],\qquad j\ge0.
\]
All occurrences in this expectation share the same variables in \eqref{p02:eq:model}.

\begin{theorem}[Rank-two mixed moments]\label{p02:thm:mixed}
For \(r=2\) and every integer \(m\ge1\),
\begin{align*}
 M_0&=\frac6m, &M_1&=\frac{114}{m^2},\\
 M_2&=\frac{66}{m^2}+\frac{4740}{m^3},\\
 M_3&=\frac{5358}{m^3}+\frac{342756}{m^4},\\
 M_4&=\frac{1962}{m^3}+\frac{508620}{m^4}+\frac{37984464}{m^5}.
\end{align*}
Also
\begin{equation}\label{p02:eq:tailmoment}
 \E\tr \Delta^4=\frac{654}{m^2}+\frac{36924}{m^3}.
\end{equation}
These identities concern polynomials and remain valid even when \(m<r\), where the inverse penalty is not defined as an ordinary inverse.
\end{theorem}

\subsection{The live residual label}
Condition on all \(g_i,h_i\). Since the \(\xi_i\)'s are independent standard Gaussians,
\begin{equation}\label{p02:eq:marked}
 \E_{\xi}[cc^{\mathsf T}\mid g,h]
 =D:=\frac1{m^2}\sum_i w_i^2g_ig_i^{\mathsf T},\qquad
 M_j=\E\tr(\Delta^jD).
\end{equation}
This is the ordinary conditional covariance calculation for a linear
Gaussian response; it is not a new covariance identity. In particular
$\E_\xi F=\tr(\Gamma^{-1}D\Gamma^{-1})$ when the Gram is invertible.
What the mixed calculation retains is the dependence of this covariance
on the same design rows. It does not make \(D\) independent of \(\Delta\). In particular the same sample label in \(D\) may recur in every following factor of \(\Delta\).

\subsection{First three moments with general radial weights}
The next calculation works for any nonnegative iid \(w_i\), independent of the Gaussians, with \(\E w_i=1\) and moments \(\mu_k=\E w_i^k\). In our model \(\mu_k=(2k-1)!!\).
Put \(X_i=w_ig_ig_i^{\mathsf T}-I\) and \(D_i=w_i^2g_ig_i^{\mathsf T}\). Gaussian radial moments give
\[
 \E D_i=\mu_2 I,\qquad
 \E X_i^2=v_r I,\quad v_r=(r+2)\mu_2-1.
\]
The terms with a sample label appearing just once in an \(X\)-position vanish. Consequently
\begin{align}\label{p02:eq:firstthree}
 M_0&=\frac{r\mu_2}{m},\nonumber\\
 M_1&=\frac{r[(r+2)\mu_3-\mu_2]}{m^2},\\
 M_2&=\frac{r}{m^3}\left\{(m-1)\mu_2v_r
 +(r+2)(r+4)\mu_4-2(r+2)\mu_3+\mu_2\right\}.\nonumber
\end{align}
For example, in \(M_2\), either the two \(X\)-labels agree and differ from the \(D\)-label, or all three labels agree. These are exactly the two terms in braces. Substitution of \(\mu_2=3,\mu_3=15,\mu_4=105\) proves the first three lines of Theorem~\ref{p02:thm:mixed}.

\subsection{A finite Wick formula for all the required coefficients}
Let \(\mathcal P(L)\) be the set of set partitions of \(\{0,\ldots,L-1\}\), and let \(\fall m k=m(m-1)\cdots(m-k+1)\). For a partition \(\pi\), attach independent standard Gaussian vectors to its blocks. Write
\[
 \Phi_r(\pi)=\E\tr\prod_{t=0}^{L-1}g_{\pi(t)}g_{\pi(t)}^{\mathsf T}.
\]
The sample partitions are the classical moment bookkeeping underlying the
moment--cumulant calculus \cite[Chapter~2]{McCullagh}.
Wick--Isserlis contraction \cite{Isserlis} gives an exact integer polynomial \(\Phi_r\). More explicitly, the two occurrences at position \(t\) have coordinate indices \(t\) and \(t+1\pmod L\). Pair Gaussian occurrences only within each block of \(\pi\). Each pairing equates its two coordinate indices. If the resulting equivalence relation has \(c\) coordinate classes, its contribution is \(r^c\). Summing these contributions defines \(\Phi_r(\pi)\) without any analytic approximation.

Writing \(T_l=\E\tr(\Gamma^lD)\), expansion of the original sample labels yields
\begin{equation}\label{p02:eq:wick}
 T_l=\frac1{m^{l+2}}
 \sum_{\pi\in\mathcal P(l+1)}\fall m {\lvert\pi\rvert}
 \Phi_r(\pi)\prod_{B\in\pi}\mu_{\lvert B\rvert+\mathbf1_{\{0\in B\}}},
 \qquad
 M_j=\sum_{l=0}^j(-1)^{j-l}\binom jl T_l.
\end{equation}
The extra unit in the moment order of the marked block comes from \(w_0^2\) rather than \(w_0\). It is exactly the dependence that an independent-copy replacement loses.

For \(r=2\), group the terms in \eqref{p02:eq:wick} by the number \(k\) of distinct sample labels. The coefficient of \(\fall m k/m^{l+2}\) in \(T_l\) is as follows.
\begin{center}
\begin{tabular}{c r r r r r}
\toprule
\(l\backslash k\)&1&2&3&4&5\\\midrule
0&6&&&&\\
1&120&6&&&\\
2&5040&312&6&&\\
3&362880&21240&576&6&\\
4&39916800&2041200&57096&912&6\\\bottomrule
\end{tabular}
\end{center}
This is a finite Wick evaluation on at most five sample positions. For example, the one-label coefficient is
\(\mu_{l+2}\E\norm g^{2l+2}=(2l+3)!\).
For \(l=2,k=2\), the three partitions contribute \(120+120+72=312\).
The stated coordinate-equivalence rule computes every other entry using only pairings and integer arithmetic. Substituting this table into \eqref{p02:eq:wick} gives the five mixed identities. The accompanying script implements exactly this finite rule and verifies independent small-instance Gaussian-quadrature checks.

\subsection{The fourth Gram moment}
For independent centered matrix copies, the four-position label expansion gives
\[
 \E\tr \Delta^4=\frac{a_r+(m-1)b_r}{m^3},
\]
where \(a_r=\E\tr X^4\),
\(b_r=2\tr V^2+\E\tr(XX'XX')\), and \(V=\E X^2\).
In the current model,
\begin{align*}
 V&=(3r+5)I,\\
 a_r&=r\{105(r+2)(r+4)(r+6)-60(r+2)(r+4)+18(r+2)-3\},\\
 b_r&=r(18r^2+81r+93).
\end{align*}
To check the alternating term without commuting matrices, for every symmetric \(H\),
\[
 \E[XHX]=5H+3(\tr H)I.
\]
It follows that the alternating contribution is
\(5r(3r+5)+3[3r(r+2)-r^2]\).
For \(r=2\), \(a_r=37578\) and \(b_r=654\), proving \eqref{p02:eq:tailmoment}. This is the \emph{same} ordered independent-sample identity used in
Section~\ref{sec:shared}, now with different one-sample moments.
The component-reuse interface in \cite{Framework} organizes this classical
sample expansion; neither the partition counting nor Wick pairing is new.

\par\Needspace{10\baselineskip}
\section{The Wick--Hermite interface for the mixed chain}\label{p02:sec:interface}
Use the original Gaussian probability space of all \((h_i,g_i,\xi_i)\), and let \(\mathcal H=\R^r\otimes L^2\). Let \(\mathcal M_\Delta\) be matrix multiplication by \(\Delta\), and let \(c=\mathcal M_c\Omega\) be the actual vector-polynomial seed. Then
\[
 M_j=\langle c,\mathcal M_\Delta^j c\rangle_{\mathcal H}.
\]
In a normalized Hermite basis, scalar coordinate multiplication obeys
\[
 xH_k=\sqrt{k+1}H_{k+1}+\sqrt k H_{k-1}.
\]
The seed has total degree at most four and involves at most one sample per term. Multiplication by \(\Delta\) raises degree by at most four and activates at most one further sample.
Let \(\Pi\) retain total degree at most twelve and sample support at most three; one may additionally retain the exact odd residual-coordinate sector and even \(h_i\)-parities. With
\(K=\Pi\mathcal M_\Delta\Pi\), both \(Kc=\mathcal M_Ec\) and \(K^2c=\mathcal M_\Delta^2c\) hold exactly. Since \(K\) is self-adjoint,
\begin{equation}\label{p02:eq:seeded}
 M_j=\langle c,K^jc\rangle\quad(0\le j\le4),\qquad
 M_2=\norm{Kc}^2,\quad M_4=\norm{K^2c}^2.
\end{equation}
The odd moments are the corresponding cross inner products. The degree-twelve cutoff suffices because only two forward steps are needed to form the fourth seeded moment; its polynomial expectation contains primitive moments up to degree twenty-four. This is the finite Hermite compression principle in \cite{Framework}, with a
non-vacuum seed. The Hermite recurrence and Gaussian chaos interpretation are
classical \cite{Janson}. Exact finite compression is used here, not a bound on
the full Gaussian multiplication operator.

The \(\xi_i\)-label is preserved under every \(\Delta\)-insertion because \(\Delta\) contains no \(\xi\)-variables. Its endpoint contraction is \eqref{p02:eq:marked}. Wick's rule then evaluates the retained coordinate Grams and sample-label partitions in \eqref{p02:eq:wick}. Finally, the polynomial functional calculus in the next section converts these mixed energies to a reconstruction bound.

No global multiplication norm is estimated. No positive-coefficient cone is assumed: the inverse polynomial has signed coefficients. Wick and the Hermite calculus are compatible exact representations; the framework is not claimed to be mathematically necessary for these finite calculations. Its concrete role here is to retain the seed, its live sample label, and the signed mixed terms until the final valid spectral bound.

\par\Needspace{10\baselineskip}
\section{Signed finite remainder and good-event certificates}\label{p02:sec:good}
Let \(\mathcal A_\theta=\{\norm \Delta\le\theta\}\), where \(0<\theta<1\), and define
\[
 p_3(x)=1-2x+3x^2-4x^3,\qquad
 b_\theta=\frac{5-4\theta}{(1-\theta)^2},\qquad
 q_\theta(x)=p_3(x)+b_\theta x^4.
\]
The finite scalar identity and its use on a self-adjoint spectrum are
ordinary polynomial algebra and spectral functional calculus; see
\cite[Chapter~1]{Higham} for the matrix-function background.
They are not a new inverse-moment theorem. The exact finite identity is
\begin{equation}\label{p02:eq:remainder3}
 \frac1{(1+x)^2}=p_3(x)+x^4\frac{5+4x}{(1+x)^2}.
\end{equation}
The final quotient is decreasing for \(x>-1\), so on \([-\theta,\theta]\),
\[
 (1+x)^{-2}\le q_\theta(x).
\]
Importantly, \(q_\theta\) is nonnegative on \emph{the whole real line}, not merely on the good interval. Indeed \(b_\theta\ge5\) and
\[
 p_4(x):=p_3(x)+5x^4
 =5\left(x^2-\frac25x\right)^2+
 \frac{11}5\left(x-\frac5{11}\right)^2+\frac6{11}>0.
\]
Therefore functional calculus gives
\begin{equation}\label{p02:eq:good}
 \E[F\mathbf1_{\mathcal A_\theta}]
 \le M_0-2M_1+3M_2-4M_3+b_\theta M_4.
\end{equation}
The global positivity is what permits dropping the indicator from the polynomial on the right; no conditioning-independence step is used.

\begin{theorem}[Explicit rank-two reconstruction certificate]\label{p02:thm:restart}
For \(r=2\), choose \(\theta=1/2\), and define
\begin{align*}
 d_m&=\frac{10464}{m^2}+\frac{590784}{m^3},\\
 u_m&=\frac6m-\frac{30}{m^2}+\frac{16332}{m^3}
       +\frac{4732416}{m^4}+\frac{455813568}{m^5}.
\end{align*}
Then \(\PP(\mathcal A_{1/2}^c)\le d_m\) and
\(\E[F\mathbf1_{\mathcal A_{1/2}}]\le u_m\).
If independent reconstruction sketches are drawn until \(\mathcal A_{1/2}\) holds and \(d_m<1\), then the accepted reconstruction satisfies
\begin{equation}\label{p02:eq:restart}
 \E F_{\rm accepted}\le\frac{u_m}{1-d_m},\qquad
 \E[\text{number of attempts}]\le\frac1{1-d_m}.
\end{equation}
The corresponding total squared error is at most \(1+u_m/(1-d_m)\) for unit residual energy.
\end{theorem}
\begin{proof}
Markov's inequality and \eqref{p02:eq:tailmoment} give
\(\PP(\mathcal A_{1/2}^c)\le16\E\tr \Delta^4=d_m\).
Here \(b_{1/2}=12\), so substitution of Theorem~\ref{p02:thm:mixed} into \eqref{p02:eq:good} gives \(u_m\). The accepted draw has the original distribution conditioned on \(\mathcal A_{1/2}\), and its waiting time is geometric. Dividing by the acceptance probability proves \eqref{p02:eq:restart}.
\end{proof}

A restart is an explicitly declared algorithmic variant. It requires another reconstruction sketch or precomputed independent candidates; it is not free in a strict single-pass model. The good-event inequality itself applies to the original unchanged sketch. The next section treats its unconditional expectation without restart.

\subsection{Comparison on the same good event}
The separate-norm argument gives only
\[
 F\mathbf1_{\mathcal A_{1/2}}\le4\norm c^2\mathbf1_{\mathcal A_{1/2}},
 \qquad \E F_{\rm accepted}\le\frac{24}{m(1-d_m)}.
\]
The following are outward-rounded upper bounds; all underlying comparisons use exact rational arithmetic.
\begin{center}
\begin{tabular}{r r r r}
\toprule
\(m\)&Bad-event bound&Mixed accepted penalty&Separate-norm penalty\\\midrule
500&0.046583&0.012693&0.050346\\
1000&0.011055&0.006059&0.024269\\
5000&0.000424&0.001200&0.004803\\
10000&0.000106&0.000600&0.002401\\\bottomrule
\end{tabular}
\end{center}
The comparison fixes the same acceptance event and probability accounting. The near-factor-four difference is not an impossibility theorem for every optimized norm-based proof: a different threshold can improve a norm certificate. The more invariant distinction is that norm separation cannot reproduce the signed mixed coefficients.

\par\Needspace{10\baselineskip}
\section{Unconditional expectation: a separate inverse bound}\label{p02:sec:inverse}
We now justify an expansion for the \emph{original unrestarted} reconstruction. This step does not follow from positive moments alone.

\begin{lemma}[Uniform inverse eighth moment]\label{p02:lem:inverse}
Put
\[
 c_*:=\E|h|^{-1/2}=2^{-1/4}\frac{\Gamma(1/4)}{\sqrt\pi},\qquad
 L_r:=2r(r+2)^5c_*^4.
\]
For every integer \(m\ge32(r+2)\),
\begin{equation}\label{p02:eq:inverse}
 \bigl(\E\norm{\Gamma^{-1}}^8\bigr)^{1/8}\le L_r.
\end{equation}
The constants are deliberately crude and are not used in the numerical table.
The Wishart input is classical; the elementary batching argument below supplies
a sufficient bound for this weighted model, not an optimal integrability threshold.
\end{lemma}
\begin{proof}
Let \(b=r+2\), and form a batch \(S_b=\sum_{i=1}^b h_i^2g_ig_i^{\mathsf T}\), with \(s_b=\lambda_{\min}(S_b)\). If \(W_b=\sum_{i=1}^b g_ig_i^{\mathsf T}\), then
\[
 S_b\succeq\min_i h_i^2\,W_b.
\]
Thus, by independence and concavity,
\[
 \E s_b^{-1/4}
 \le b c_*\bigl(\E\tr W_b^{-1}\bigr)^{1/4}
 =b c_*r^{1/4}.
\]
The last expectation is the standard inverse-Wishart first moment
\cite[Proposition~10.2]{HMT} (also \cite[Fact~5.3]{EW}).
For completeness, its value is \(r\): the reciprocal of a diagonal entry of \(W_b^{-1}\) is a Gaussian-column residual squared norm with \(b-r+1=3\) degrees of freedom, whose reciprocal expectation is one. This follows by the Schur complement and the elementary chi-square integral.

Take 32 independent batches. The smallest eigenvalue of their sum is at least the sum of their smallest eigenvalues. Arithmetic--geometric mean gives
\[
 \E\left\|\left(\frac1{32b}\sum_{i=1}^{32b}h_i^2g_ig_i^{\mathsf T}\right)^{-1}\right\|^8
 \le b^8(\E s_b^{-1/4})^{32}
 \le b^{40}c_*^{32}r^8.
\]
For general \(m\ge32b\), split off complete blocks of size \(32b\) and drop the nonnegative remainder. These complete blocks retain at least half the mass. Their average has smallest eigenvalue at least the average of their individual smallest eigenvalues; convexity of \(x\mapsto x^{-8}\) then supplies at most another factor \(2^8\). Taking an eighth root proves \eqref{p02:eq:inverse}.
\end{proof}

\subsection{Explicit positive-moment constants}
For even \(p\), let \(\beta_p\) count set partitions of \(p\) positions with no singleton blocks, and write
\[
 \nu_p=(2p-1)!!,\qquad R_k(r)=\prod_{a=0}^{k-1}(r+2a).
\]
In particular \(\beta_8=715\), \(\beta_{10}=17722\), and \(\beta_{12}=580317\).
Define
\begin{align*}
 e_p&=\beta_p^{1/p}\left[(\nu_pR_p(r))^{1/p}+\sqrt r\right],\\
 c_p&=\left[\beta_p\nu_pR_{p/2}(r)(p-1)!!\right]^{1/p}.
\end{align*}
Then
\begin{equation}\label{p02:eq:momentconstants}
 \bigl\|\norm \Delta\bigr\|_{L_p}\le\frac{e_p}{\sqrt m},\qquad
 \bigl\|\norm c\bigr\|_{L_p}\le\frac{c_p}{\sqrt m}.
\end{equation}
To verify this, for independent identically distributed centered vectors \(Z_i\) in any real Hilbert space, expand \(\norm{\sum_i Z_i}^{p}\) as a product of inner products. A singleton sample label vanishes. Each surviving term has absolute expectation at most \(\E\norm{Z_1}^p\), and at most \(p/2\) distinct labels. There are at most \(\beta_pm^{p/2}\) such assignments. Hence
\[
 \E\left\|\frac1m\sum_i Z_i\right\|^p
 \le\beta_pm^{-p/2}\E\norm{Z_1}^p.
\]
Apply this in Frobenius space to \(Z_i=X_i\), using
\(\Frob{X_i}\le w_i\norm{g_i}^2+\sqrt r\), and to \(Z_i=w_ig_i\xi_i\). Their local moments are exactly bounded by the expressions defining \(e_p,c_p\).

\begin{theorem}[Unconditional expansion with finite remainder]\label{p02:thm:unconditional}
Let
\(P_4(m,r)=\sum_{j=0}^4(-1)^j(j+1)M_j\).
For \(m\ge32(r+2)\), the original unrestarted penalty is integrable and
\begin{equation}\label{p02:eq:uncondrem}
 \left|\E F-P_4(m,r)\right|
 \le L_r^2c_8^2\left(\frac{6e_{10}^5}{m^{7/2}}+\frac{5e_{12}^6}{m^4}\right).
\end{equation}
In particular, for each fixed \(r\),
\begin{equation}\label{p02:eq:asymptotic}
 \E F=\frac{3r}{m}-\frac{3r(r+3)}{m^2}
 +\frac{r(45r^2+324r+471)}{m^3}+O_r(m^{-7/2}).
\end{equation}
For \(r=2\), this is
\[
 \E F=\frac6m-\frac{30}{m^2}+\frac{2598}{m^3}+O(m^{-7/2}).
\]
The remainder is a fixed-rank estimate, not a sharp bound uniform in growing \(r\).
\end{theorem}
\begin{proof}
For every \(x>-1\),
\[
 ...
 =-x^5\frac{6+5x}{(1+x)^2}.
\]
Since \(\Gamma\) is positive definite, this is a globally valid matrix identity on its spectrum. Therefore
\[
 |F-c^{\mathsf T}p_4(\Delta)c|
 \le\norm{\Gamma^{-1}}^2\norm c^2(6\norm \Delta^5+5\norm \Delta^6).
\]
Apply H\"older with exponents \(4,4,2\), followed by \eqref{p02:eq:inverse} and \eqref{p02:eq:momentconstants}. This proves \eqref{p02:eq:uncondrem} and also integrability. The exact polynomial in Appendix~\ref{p02:app:general} gives \eqref{p02:eq:asymptotic}.
\end{proof}

\par\Needspace{10\baselineskip}
\section{Comparison, cancellation, and scope of the mixed result}\label{p02:sec:scope}
For rank two, the true mixed correction begins
\[
 M_0-2M_1+3M_2=\frac6m-\frac{30}{m^2}+\frac{14220}{m^3}.
\]
An independent-copy replacement of \(\Delta\) makes \(M_1\) zero. It leaves the leading term of \(M_2\) equal to \(66/m^2\), and therefore gives the false second-order prediction \(6/m+198/m^2\) for the original problem. The lost contribution is \(228/m^2\). The true original-estimator expansion in Theorem~\ref{p02:thm:unconditional} verifies the negative sign without relying on good-event conditioning.

For comparison, the exact fully Gaussian benchmark is
\[
 \E F_{\rm Gaussian}=\frac2{m-3}
 =\frac2m+\frac6{m^2}+\frac{18}{m^3}+O(m^{-4})
\]
for rank two \cite[Lemma~A.5]{TYUC2017}; see also \cite[Lemma~1 in arXiv v2]{BP2020} and \cite[Theorem~2.1]{EW}. The qualitative fact that weighting cannot improve this conditional
Gaussian prediction risk also has an established explanation. Let $Z_0$
have rows $g_i^{\mathsf T}$ and let $H=\diag(h_1,\ldots,h_m)$, invertible
almost surely. Then
\[
 \E_\xi F=\norm{(HZ_0)^\dagger H}_F^2
 \ge\norm{Z_0^\dagger}_F^2
\]
by the ``weighting hurts'' inequality \cite[Lemma~5.8]{EW}.
Taking expectations gives $\E F\ge r/(m-r-1)$ for $m>r+1$,
with the inequality understood in the extended sense if needed.
That comparison does not supply the mixed coefficients in
Theorem~\ref{p02:thm:unconditional}. The common-factor geometry has leading inflation three. This does not contradict the empirical universality observations for other tested embeddings in that paper, and it does not decide Conjecture~7.1 (all-algorithm optimality) or the separate generalized-Nystr\"om
nonoptimality question in \cite[Section~7]{EW}.

The bounded P02 task is resolved here for a specified structured sketch and subspace--residual family: exact mixed terms, signed finite-remainder control, a nonasymptotic good-event/restart certificate, and a separately justified original-law unconditional expansion. No general inverse-moment calculus, arbitrary-geometry universality theorem, optimal row count, or publication-priority claim is made.



\begin{takeaway}[title={What Chain C adds to the two-proof comparison}]
For the rMPS scalar extremum, Wick and conditional energy gave the same
constants. In this mixed problem their roles are complementary: the Hermite
compression keeps an exact seeded moment, Wick evaluates its marked sample
partitions, and a nonnegative polynomial majorant preserves the signed
combination. Separate norms give a valid but coarser bound on the same event.
No comparison here shows that Wick alone cannot evaluate these moments, or
that the framework alone supplies inverse integrability.
\end{takeaway}
...
\paragraph{Checks repeated for this integration.}
The original rMPS checker is not part of the supplied source bundle, so its
reported original-core quadrature experiments have not been rerun in this
integration. The included integration checker independently verifies the
rational rMPS certificates and local pairing row sums. It additionally runs
the supplied P02 checker and a separate seeded Hermite calculation; see
Appendix~\ref{p02:app:checks}.
\clearpage

\section{Mixed reconstruction: exact general-rank polynomials}\label{p02:app:general}
For the same common-factor family with any \(r\ge1\), equation~\eqref{p02:eq:wick} gives
\begin{align*}
 M_0={}&\frac{3r}{m},\\
 M_1={}&\frac{r(15r+27)}{m^2},\\
 M_2={}&\frac{r(9r+15)}{m^2}
       +\frac{r(105r^2+591r+768)}{m^3},\\
 M_3={}&\frac{r(135r^2+666r+807)}{m^3}\\
       &+\frac{r(945r^3+10890r^2+39069r+42120)}{m^4},\\
 M_4={}&\frac{r(54r^2+243r+279)}{m^3}\\
       &+\frac{r(1710r^3+17253r^2+56757r+58104)}{m^4}\\
       &+\frac{r(10395r^4+202410r^3+1393263r^2+3938400r+3756780)}{m^5}.
\end{align*}
Also
\[
 \E\tr \Delta^4=\frac{r(18r^2+81r+93)}{m^2}
 +\frac{r(105r^3+1182r^2+4197r+4500)}{m^3}.
\]
The polynomial used in Theorem~\ref{p02:thm:unconditional} is
\begin{align*}
 P_4(m,r)={}&\frac{3r}{m}-\frac{3r(r+3)}{m^2}
 +\frac{r(45r^2+324r+471)}{m^3}\\
 &+\frac{r(4770r^3+42705r^2+127509r+122040)}{m^4}\\
 &+\frac{5r(10395r^4+202410r^3+1393263r^2+3938400r+3756780)}{m^5}.
\end{align*}
For \(r=2\),
\[
 P_4(m,2)=\frac6m-\frac{30}{m^2}+\frac{2598}{m^3}
 +\frac{1172076}{m^4}+\frac{189922320}{m^5}.
\]

\clearpage
\section{Mixed reconstruction: reproducibility and check limits}\label{p02:app:checks}
The original accompanying \texttt{p02\_mixed\_reconstruction\_checks.py}
is included unchanged in the reproducibility bundle. It:
\begin{enumerate}[itemsep=2pt]
\item enumerates the exact sample partitions and Wick pairings in \eqref{p02:eq:wick};
\item checks the general-rank single-sample formulas against direct chi-square moments;
\item independently integrates rank-two, one- and two-sample polynomial moments with seven-node Gauss--Hermite quadrature in each remaining Gaussian coordinate, after exact integration of the residual variables;
\item verifies the scalar remainder and sum-of-squares identities symbolically;
\item computes the restart and separate-norm certificates as exact rational numbers.
\end{enumerate}
The node count is sufficient because the largest power in an individual
$h_i$ is twelve and the remaining coordinate powers are no larger; seven
Gauss--Hermite nodes integrate through degree thirteen. This is the classical
quadrature exactness property \cite{GolubWelsch}. The evaluation is numerical,
not a claim of exact rational quadrature output. The quadrature rule has
sufficient polynomial degree for these checks; floating-point evaluation produced relative discrepancies below \(2\times10^{-12}\). It is not a simulation. These checks do not by themselves verify the analytic inverse-tail argument or establish novelty; the proofs above supply the analytic claims.



The integration companion \texttt{integrated\_proof\_checks.py} adds an
independent \emph{deterministic Hermite-multiplication} calculation for $r=2$
and $m=1,2,3$. It uses probabilists' unnormalized Hermite polynomials,
\[
 x\operatorname{He}_k=\operatorname{He}_{k+1}
       +k\operatorname{He}_{k-1},\qquad
 \langle\operatorname{He}_{\alpha},\operatorname{He}_{\beta}\rangle
       =\mathbf1_{\alpha=\beta}\prod_t\alpha_t!.
\]
All coefficients and inner products are integers before division by powers
of $m$. It constructs $c$, $\Delta c$, and $\Delta^2c$ using the original
$(h_i,g_i,\xi_i)$ registers, checks the degree and active-sample budgets, and
recovers $M_0,\ldots,M_4$ as seeded inner products. This independently checks
the numerical coefficients in the Wick calculation against the multiplication
representation. It is a finite-instance check, not a formal proof for arbitrary
parameters. The companion also rechecks the original rMPS rational
probe-count tables and pairing row sums. Both scripts state their requirements
and write machine-readable JSON reports.

\clearpage
\section{Attribution map and boundaries of the integrated result}
\label{p02:app:audit}
The integration preserves both original rMPS proof chains and adds the P02
calculation as a \emph{different endpoint}. Classical ingredients are cited
at their points of use. The following map separates antecedents, calculations
proved here, and statements not being made.

\begingroup\small\renewcommand{\arraystretch}{1.2}
\begin{longtable}{@{}P{.24\textwidth}P{.39\textwidth}P{.31\textwidth}@{}}
\toprule
\textbf{Ingredient}&\textbf{Attribution}&\textbf{Boundary}\\
\midrule\endfirsthead
\toprule\textbf{Ingredient}&\textbf{Attribution}&\textbf{Boundary}\\\midrule\endhead
Gaussian tensor-train projections and fourth-order energy iteration &
Rakhshan--Rabusseau \cite[Theorem~1, Section~5.1]{RR2020}. &
The one-core strategy and sketch family are not introduced here.\\
Exact rMPS fourth-moment transfer and surrogate &
Cama\~no--Epperly--Meyer--Tropp \cite[Section~4]{CEMT}. &
Their stated surrogate is fourth-order; higher-order formulas here are derived,
not attributed to a published eighth-order theorem.\\
Gaussian pairing and chaos &
Isserlis \cite{Isserlis}, Goodman \cite{Goodman}, Janson \cite{Janson}. &
Wick and Hermite are compatible classical descriptions of the same Gaussian law.\\
Pairing Gram kernels; quadratic-form cumulants &
Collins--Matsumoto \cite{CollinsMatsumoto}; Lancaster and McCullagh
\cite{Lancaster,McCullagh}. &
No new Haar-integration, pairing, or cumulant theorem is claimed.\\
Original-law finite compression and retained registers &
Heidary's supplied v11 manuscript \cite{Framework}. &
An organizing interface, not sole ownership of conditioning, Wick pairing,
or spectral calculus.\\
Fixed-range reconstruction; current problem context &
Tropp et al.\ \cite[Section~4, Lemmas~A.3--A.5]{TYUC2017};
Epperly--Webber \cite[Sections~5 and~7]{EW}. &
The algorithm, decomposition, and qualitative weighting loss predate this calculation.\\
Real iid Gaussian reconstruction benchmark &
Tropp et al.\ \cite[Lemma~A.5]{TYUC2017}; Bartan--Pilanci
\cite[Lemma~1, arXiv v2]{BP2020}; Gaussian inputs in \cite{HMT}. &
The reconstruction identity predates the regression restatement and the 2026 benchmark.\\
Finite inverse polynomial and spectral majorant &
Elementary algebra; standard functional calculus, e.g.\ Higham \cite{Higham}. &
The explicit majorant is verified here. No inverse integrability follows
from polynomial moments alone.\\
P02 marked moments, constants, and weighted inverse estimate &
Derived in Sections~\ref{p02:sec:moments}--\ref{p02:sec:inverse}. &
Analytic, self-checked calculations for the stated geometry; not an exhaustive
priority claim or an optimized inverse threshold.\\
Numerical consistency and exact arithmetic &
Classical quadrature construction \cite{GolubWelsch}; bundled scripts. &
Exact symbolic/rational checks are separated from floating-point quadrature;
neither is a Lean certificate.\\
\bottomrule
\end{longtable}
\endgroup

\textbf{Version and scope notes.}
Bartan--Pilanci's accessible arXiv v2 numbers the benchmark Lemma~1,
not Lemma~2.1 as cited in \cite{EW}. The supplied v11 spells its author's
name ``Diar Heidary''; that spelling is retained. This source-and-role audit
is not an exhaustive priority search or a proof-assistant certificate.
...
called Wick's rule. It is the local Gaussian input to Chain~A and the marked-sample moment
evaluation in Chain~C.
...
\textit{Role:} exact finite moment compression, retained sample registers, and the
original-law quadratic-energy interface with component reuse. The author and title are
...
\bibitem{EW}
E.~N.~Epperly and R.~J.~Webber,
\emph{Sharp analysis of sketched least squares and randomized low-rank approximation},
2026. \href{https://arxiv.org/abs/2605.19096v2}{arXiv:2605.19096v2}.
\textit{Role:} Theorem~2.1 for sharp Gaussian/orthonormal benchmarks;
Section~5.1 for the reconstruction error decomposition; Section~5.2 for
classical Wishart inputs; Section~7 for the distinct universality and
all-algorithm optimality questions. The earlier real regression formula is
credited there to Bartan--Pilanci. The matching fixed-range reconstruction
identity already appears in Tropp et al.\ (2017), also cited directly here. The present
fixed-geometry calculation does not resolve the Section~7 conjectures.

\bibitem{BP2020}
B.~Bartan and M.~Pilanci,
\emph{Distributed Sketching Methods for Privacy Preserving Regression},
2020. \href{https://arxiv.org/abs/2002.06538v2}{arXiv:2002.06538v2}.
\textit{Role:} Lemma~1 in this version gives the exact real Gaussian
sketch-and-solve prediction-error factor $r/(m-r-1)$; its proof uses the
inverse-Wishart expectation. This is a directly checked regression antecedent of the
2026 benchmark, not the earliest fixed-range reconstruction formula and not a
claim about the weighted Khatri--Rao calculation.
...
\href{https://arxiv.org/abs/0909.4061v2}{arXiv:0909.4061v2}.
\textit{Role:} Propositions~10.1--10.2 give the standard Gaussian conditional
Frobenius and pseudoinverse moment ingredients. The inverse-Wishart first
moment is classical multivariate statistics, not introduced by this note or
by this survey. The weighted inverse-eighth-moment batching bound is proved here.
...
SIAM Journal on Matrix Analysis and Applications \textbf{38}(4) (2017),
1454--1485.
\href{https://doi.org/10.1137/17M1111590}{doi:10.1137/17M1111590};
\href{https://arxiv.org/abs/1609.00048v2}{arXiv:1609.00048v2}.
\textit{Role:} equations~(4.2)--(4.3) for $Q(\Psi Q)^\dagger\Psi A$;
Lemmas~A.3--A.4 for the error decomposition; Lemma~A.5 for the exact
conditional Gaussian reconstruction error. Remark~4.1 credits earlier
algebraically equivalent algorithms. This is a direct predecessor of the P02
setup and its Gaussian calibration, not of the weighted mixed-moment coefficients.
...
\textit{Role:} Chapter~1 for standard spectral/polynomial matrix-function
calculus. The scalar finite-remainder and sum-of-squares identities used in
Chain~C are explicitly verified in this note; no new general matrix-function
or inverse-series theorem is asserted.
