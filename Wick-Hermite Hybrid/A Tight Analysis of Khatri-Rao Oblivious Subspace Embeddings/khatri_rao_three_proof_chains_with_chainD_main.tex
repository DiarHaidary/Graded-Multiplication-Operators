...
The Wick-assisted concentration variant appears later as Chain D; it is not
substituted into A or labelled C. All application-specific proofs follow in this document.}
\clearpage
...
\bottomrule
\end{tabular}
\caption{The three principal A/B/C certificates. These are selected sufficient theorem bounds,
...
Section~\ref{sec:D}. It is not a fourth principal chain in the original A/B/C split.
\end{takeaway}
\clearpage
...
concentration variant $A^{\mathrm W}$ is presented later as Chain~D in Section~\ref{sec:D}.
...
The Wick-assisted concentration variant is separated in Section~\ref{sec:D}.
...
\raggedright
{\sffamily\bfseries\fontsize{18}{21}\selectfont Chains C and D preview\textemdash\thinspace same Wick input, different continuation}\\[1mm]
{\sffamily The next two sections use the same sharper Gaussian contractions but send them into two different proof architectures. Chain C keeps the framework route: exact Hermite/support cutoff and ordered vacuum propagation. Chain D instead keeps the concentration route: auxiliary truncation and Lieb--Tropp matrix Laplace.}\\[2mm]
\begin{center}
\makebox[\textwidth][c]{%
\resizebox{0.96\textwidth}{!}{%
\begin{tikzpicture}[
 >={Latex[length=1.6mm,width=1.2mm]},
 every node/.style={font=\fontsize{9.4}{11.4}\selectfont},
 base/.style={rounded corners=1.5pt,line width=.5pt,align=center,inner xsep=5pt,inner ysep=4pt,outer sep=.5pt},
 shared/.style={base,draw=Navy!70,fill=Ice,text width=.92\textwidth},
 cbox/.style={base,draw=Amber!85,fill=Amber!5,text width=.40\textwidth,minimum height=17mm},
 dbox/.style={base,draw=Teal!85,fill=Teal!4,text width=.40\textwidth,minimum height=17mm},
 chead/.style={base,draw=Amber,fill=Amber,text=white,text width=.40\textwidth,minimum height=10mm,font=\sffamily\bfseries\fontsize{10}{12}\selectfont},
 dhead/.style={base,draw=Teal,fill=Teal,text=white,text width=.40\textwidth,minimum height=10mm,font=\sffamily\bfseries\fontsize{10}{12}\selectfont},
 cflow/.style={->,draw=Amber!95,line width=.6pt},
 dflow/.style={->,draw=Teal!90,line width=.6pt}]

\node[shared,minimum height=18mm] (top)
 {\textbf{Shared Wick input and unchanged model}\\[2pt]
  $y_i=U^{\mathsf T}(g_i\otimes h_i),\quad Y_i=y_iy_i^{\mathsf T},\quad Z=m^{-1}\sum_i(Y_i-I_r)$\\[2pt]
  Exact radial product $\kappa_pD_{r,p}$ and mixed orthogonal fourth moments; activation proxy $4r+4$.};

\node[chead,below left=8mm and 12mm of top.south] (ch)
 {\textbf{Chain C}\\[2pt]Combined Wick--Hermite framework};
\node[dhead,below right=8mm and 12mm of top.south] (dh)
 {\textbf{Chain D}\\[2pt]Wick-assisted concentration};
\draw[cflow] (top.south)--++(-12mm,-3mm)-|(ch.north);
\draw[dflow] (top.south)--++(12mm,-3mm)-|(dh.north);

\node[cbox,below=3mm of ch] (c1)
 {\textbf{C1. Exact cutoff}\\[2pt]Total degree $\le4q$, active count $\le q$, even symmetries\\No outcome clipping};
\node[dbox,below=3mm of dh] (d1)
 {\textbf{D1. Auxiliary truncation}\\[2pt]Clip only the proof surrogate $W$ on $\{\|y\|^2\le L_s\}$\\Restore the original law at the end};
\draw[cflow] (ch)--(c1); \draw[dflow] (dh)--(d1);

\node[cbox,below=3mm of c1] (c2)
 {\textbf{C2. Local propagation costs}\\[2pt]Repeated visits use refined radial/degree budgets\\Ordered stage matrices $\widehat G_C^{[j]}$};
\node[dbox,below=3mm of d1] (d2)
 {\textbf{D2. One-row matrix moments}\\[2pt]Factorial envelope with scale $8(r+s)$\\Concentration via matrix Laplace};
\draw[cflow] (c1)--(c2); \draw[dflow] (d1)--(d2);

\node[cbox,below=3mm of c2] (c3)
 {\textbf{C3. Vacuum + Markov}\\[2pt]$r\|\widehat G_C^{[q]}\cdots\widehat G_C^{[1]}e_0\|^2$\\Original-law moment certificate};
\node[dbox,below=3mm of d2] (d3)
 {\textbf{D3. Original-law tail}\\[2pt]$U_{A^{\mathrm W}}(m,r,\delta)\le\varepsilon$\\Same sketch, concentration-type certificate};
\draw[cflow] (c2)--(c3); \draw[dflow] (d2)--(d3);

\node[shared,below=7mm of top.south |- d3.south,minimum height=14mm] (bottom)
 {\textbf{Same Wick input, two different endpoints}\\[2pt]
  C refines the framework certificate; D refines the concentration certificate.\\
  Both keep the sharp rate $m=O(\varepsilon^{-2}rt+\varepsilon^{-1}t^2)$ for $t=\log(4r/\delta)$.};
\draw[cflow] (c3.south)--++(0,-2mm)-|([xshift=-24mm]bottom.north);
\draw[dflow] (d3.south)--++(0,-2mm)-|([xshift=24mm]bottom.north);
\end{tikzpicture}%
}%
}\end{center}

\vspace{2mm}
\begingroup\fontsize{9.2}{11.2}\selectfont
\setlength{\tabcolsep}{5pt}\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}P{.18\textwidth}YY@{}}
\toprule
\textbf{Comparison}&{\color{Amber!90!black}\textbf{Chain C} (\S\ref{sec:C})}&{\color{Teal}\textbf{Chain D} (\S\ref{sec:D})}\\
\midrule
\rowcolor{Ice}
\textbf{Shared input}&Sharp radial product $\kappa_pD_{r,p}$ and mixed orthogonal fourth moments.&Same sharp Wick input.\\[3pt]
\textbf{Retained object}&Original-law Hermite walks, active labels, and local degree budgets.&One-row matrix moments of the auxiliary bounded matrix $W$ and a tail event for restoring the original law.\\[3pt]
\rowcolor{Ice}
\textbf{Assembly}&Positive sector majorant and ordered vacuum propagation, then Markov.&Lieb--Tropp matrix Laplace with deterministic mean offset.\\[3pt]
\textbf{Clipping?}&No clipping at all; every outcome stays in the proof.&Only the proof surrogate is clipped, and the clipping probability and bias are charged.\\[3pt]
\rowcolor{Ice}
\textbf{Principal output}&Refined stage-dependent certificate $U_C$ from \eqref{c:eq:wick-stage-moment}.&Tail allowance $U_{A^{\mathrm W}}$ from \eqref{aw:eq:tail}.\\[3pt]
\textbf{Purpose in this document}&Third main chain, distinct from B because the local Gaussian input is sharper.&A separate section showing what concentration achieves with the same Wick input.\\
\bottomrule
\end{tabularx}
\endgroup

\vspace{2mm}
{\small At $\varepsilon=1/2$ and $\delta=0.01$, the selected sufficient row counts are $C=(12{,}800,111{,}000,1{,}250{,}000,15{,}000{,}000)$ and $D=(13{,}200,98{,}100,1{,}100{,}000,12{,}800{,}000)$ for $r=(10,100,1000,10000)$, respectively. These are theorem certificates, not optimized row counts.}
\end{minipage}%
}
\end{center}
...
\clearpage
\clearpage
\section{Chain D: Wick-assisted concentration}\label{sec:D}
\begin{takeaway}[title={A separate fourth section using the Wick input inside concentration}]
The principal comparison on pages 1--2 remains A/B/C. This section records
Chain D, namely $A^{\mathrm W}$: the same concentration architecture as A supplied with C's
better Gaussian inputs. It preserves the scale-eight result and makes the later
constants comparison fair, without silently redefining Chain A or Chain C.
\end{takeaway}
...
\begin{tabular}{@{}rrrr@{}}
\toprule
$r$&\textbf{Chain D}&\textbf{C simple}&\textbf{C refined}\\
\midrule
...
\label{tab:AWC}
\end{table}
The corresponding upward-rounded allowances for Chain~D are
...
\section{Comparing A, B, C, and D: endpoints and constants}\label{sec:comparison}
...
For the principal A/B/C comparison, define
\begin{align}
...
Section~\ref{sec:D} records a fair additional comparison after applying
...
eight supplementary certificates in Section~\ref{sec:D}, the fixed exponent
